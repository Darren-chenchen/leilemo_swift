...
class CLShareView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tipViews = UIView(frame: CGRect(x: tipX, y: APPH, width: tipW, height: tipW))
        ...
        if (!WXApi.isWXAppInstalled() && QQApiInterface.isQQInstalled()) { // 没装微信
            self.imgArr = ["share_qq", "share_qzone", "share_sina_on"]
            self.titleArr = ["QQ好友", "QQ空间", "新浪微博"]
        }
        else if !WXApi.isWXAppInstalled() && !QQApiInterface.isQQInstalled() && !(MFMessageComposeViewController.canSendText()) { //没装微信 没装q
        ...
        cancelBtn.backgroundColor = NavBackGroundColor()
        cancelBtn.setTitle("取消分享", for: .normal)
        cancelBtn.titleLabel?.font = UIFont.systemFont(ofSize: 14)
        CLViewsBorder(cancelBtn, borderWidth: 0, borderColor: NavBackGroundColor(), cornerRadius: 3)
    ...
    func clickCancel() {
        UIView.animate(withDuration: 0.25, animations: {
            ...
            self.tipViews.frame = CGRect(x: 0, y: tipY, width: APPW, height: 0)
        }) { (finished: Bool) in
            let win = UIApplication.shared.keyWindow!
    ...
    func initWithBtn(_ btn: UIButton) {
        ...
        shareParams.ssdkSetupShareParams(byText: self.shareContent, images: UIImage(named:self.shareImageStr), url: NSURL(string:urlStr) as URL!, title: self.shareTitle, type: SSDKContentType.auto)
